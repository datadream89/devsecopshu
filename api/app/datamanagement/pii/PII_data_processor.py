...
def import_dataset(dataset_path_var, messages_pipe = None):
    ...
    try:
        ...
        elif dataset_path_l.endswith('txt'):
...
def initialize_lists(function_pipe = None):
    ...
    restricted_location = ["zipcode", "zip","district", "country", "subcountry", "parish", "lc", "village", "community", "address", "gps", "lat", "log", "coord", "location", "house","compound", "panchayat", "name", "fname", "lname", "first_name", "last_name", "birth", "birthday", "bday", ]
    ...
    # Additions
    restricted_expansions = ['credit_score', 'credit_date', 'name', 'insurance', 'medical', 'number', 'enumerator', 'rand', 'random', 'child_age', 'uid', 'latitude', 'longitude', 'coordinates', 'web', 'website', 'hh', 'address', 'age', 'nickname', 'nick_name', 'firstname', 'lastname', 'sublocation', 'alternativecontact', 'division', 'gps', 'resp_name', 'resp_phone', 'head_name', 'headname', 'respname', 'subvillage', 'survey_location']
...
def word_match_stemming(possible_pii, restricted, cols, stemmer, label_dict, sensitivity = 3, function_pipe = None, messages_pipe = None):
# Looks for matches between variable names, variable name stems, restricted words, and restricted word stems
    ...
    for v in cols: #for v in tqdm(dataset.columns):
        for r in restricted:
    ...
    # breakpoint()
    return possible_pii, function_pipe
...
def fuzzy_partial_stem_match(possible_pii, restricted, cols, stemmer, threshold = 0.75, function_pipe = None, messages_pipe = None):
...
# Recommended value is 0.75. Higher numbers (i.e. 4) will identify more possible PII, while lower numbers (i.e. 0.5) will identify less potential PII.
    ...
    for v in tqdm(cols): #for v in tqdm(dataset.columns):
    ...
    # smart_return(possible_pii, function_pipe)
    # breakpoint()
    return list(set(possible_pii)), function_pipe
...
if __name__ == "__main__":
    ...
    # df = pd.read_csv("raw/tax.txt", sep='|')  #pd.read_csv('raw/credit_scores.txt', sep='|')
    # identified_pii = []
    # restricted_vars = initialize_lists()
    # restricted_vars, stemmer = stem_restricted(restricted_vars)

    # identified_pii,_ = word_match_stemming(identified_pii, restricted_vars, df, stemmer, False)
    # identified_pii,_ = fuzzy_partial_stem_match(identified_pii, restricted_vars, df, stemmer, threshold=0.75)
    # identified_pii,_ = unique_entries(identified_pii, df, min_entries_threshold=0.5)
    # identified_pii,_ = date_detection(identified_pii, df)

    # reviewed_pii, removed_status = review_potential_pii(identified_pii, df)
    # dataset, recoded_fields = recode(df)
    # path, export_status = export(df)
    # log(reviewed_pii, removed_status, recoded_fields, path, export_status)
    ...
    def string_match(cols, look_up_word, threshold=0.9):
        identified_pii = []
    ...
    match = {col_name : string_match(BUSINESS_GLOSSARY, col_name) for col_name in col_names}
    print(match)
